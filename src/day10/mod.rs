...
pub fn _main(data: PathBuf, out: PathBuf, verbosity: u8) -> Result<()> {
    let height_map = parse(data)?;
    if verbosity > 2 {
        print_heightmap(&height_map);
    }
    let res1 = solve(&height_map);
    println!("res1: {res1}");
    Ok(())
}
...
fn get_paths(heights: &HeightMap, start: Position) -> Vec<Position> {
    if heights[start.0][start.1] == 9 {
        return vec![start];
    }
    let mut tot = HashSet::new();
    for next_start in next_path(heights, start) {
        for p in get_paths(heights, next_start) {
            tot.insert(p);
        }
    }
...
fn solve(heights: &HeightMap) -> usize {
    let mut tot = 0;
    for (i, row) in heights.iter().enumerate() {
        for (j, height) in row.iter().enumerate() {
            if *height == 0 {
                tot += get_paths(heights, (i, j)).len();
            }
        }
    }
    tot
